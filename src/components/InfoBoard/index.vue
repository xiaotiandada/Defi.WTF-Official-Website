<template>
  <div @mouseenter="animate" @mouseleave="animate" class="board">
    <img :src="get(ad, 'content')" :alt="'ad0'" />
    <div v-if="animState" class="animated bounceInDown info-container">
      <div class="info">
        <div class="left">
          <div class="label">price:</div>
          <span class="num">{{ get(ad, 'price') }}</span>
          <div v-if="ad.id === 'ad0'" class="label">owner:</div>
          <div v-if="ad.id === 'ad0'">5Grwv......GKutQY</div>
          <!-- <div class="label">taxes ratio: </div>
          <span class="num">{{ get(ad, 'ratio') }}</span> / day-->
        </div>
        <div class="right">
          <div v-if="ad.owner" class="btn" @click="onClick">Update My Billboard</div>
          <div v-else class="btn" @click="onClick">Buy It !</div>
        </div>
      </div>
    </div>

    <b-modal :title="dialogTitle" id="buy" class="modal">
      <b-form ref="form" label-position="top" @submit="bid">
        ...
        <b-form-group label="Pre payed tax">
          <b-form-input v-model="form.prePay" placeholder="Pre payed tax" type="number" clearable></b-form-input>
        </b-form-group>
    ...
    <b-modal :title="dialogTitle" id="update" class="modal">
      <b-form ref="form" label-position="top" @submit="setBoard">
        <div class="row-label">Taxes Ratio</div>
        <div class="row-data">
          <span>{{ get(ad, 'ratio') }}</span> / day
        </div>
        <b-form-group label="Price">
          <b-form-input v-model="form.price" placeholder="Price" type="number" clearable></b-form-input>
        </b-form-group>
        <b-form-group label="Pre payed tax">
          <b-form-input v-model="form.prePay" placeholder="Pre payed tax" type="number" clearable></b-form-input>
        </b-form-group>
      ...
      <span slot="footer" class="dialog-footer">
        <b-button round @click="showDialogPut = false">Cancel</b-button>
        <b-button round @click="setBoard" class="btn-ok">Ok</b-button>
      </span>
    </b-modal>
  </div>
</template>
